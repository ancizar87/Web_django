...
class ThreadTestCase(TestCase):
    ...
    def test_filter_non_exitstent_thread(self):
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)
        self.assertEqual(len(threads), 0)
        
    def test_add_messages_to_thread(self):
        self.thread.users.add(self.user1,self.user2)
        message1 = Message.objects.create(user=self.user1, content="hola como va todo")
        message2 = Message.objects.create(user=self.user2, content="hola todo bn")
    ...
    def test_add_message_from_user_not_in_thread(self):
        ...
        message1 = Message.objects.create(user=self.user1, content="hola como va todo")
        message2 = Message.objects.create(user=self.user2, content="hola todo bn")
        message3 = Message.objects.create(user=self.user3, content="hola me meti por error")
